# -*- coding: utf-8 -*-
"""Project 0 Done.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/128aXC4EGsEWE85JTN0T-6xVzFXgJSowX
...
def predict_using(network):
    def predictor(raw_data):
        ...
        with torch.no_grad(): # turns off automatic differentiation, which isn't required but helps save memory
            ...
            network.eval()
            preprocessed_data = make_matrices(raw_data)
            data_loader = torch.utils.data.DataLoader(preprocessed_data, batch_size=16)

            predictions = []
            for chars_batch, mask_batch, _ in data_loader:
              chars_batch, mask_batch = chars_batch.cuda(), mask_batch.cuda()
              outputs = network(chars_batch, mask_batch)
              argmax_indices = outputs.argmax(dim=1)
              predictions.extend( [all_tags[i] for i in argmax_indices] )
...
def train(network, n_epochs=25):
    ...
    preprocessed_train_data = make_matrices(train_data_raw)
    data_loader = torch.utils.data.DataLoader(preprocessed_train_data, batch_size=16, shuffle=True)
    network = network.cuda()
    criterion = torch.nn.CrossEntropyLoss()
    optimizer = torch.optim.Adam(network.parameters())
    
    predictor = predict_using(network)
    previous_best_score = -9999
    ...
    for epoch in range(n_epochs):
        ...
        if validation_score > previous_best_score:
          previous_best_score = validation_score
          torch.save(network.state_dict(), "models/best_model")

    # YOUR CODE HERE
    # * load the best model from the file where you saved it using `torch.load` and `network.load_state_dict`
    #   and return it

    network.load_state_dict(torch.load("models/best_model"))
    return network
...
class PoolingNetwork(nn.Module):
    def __init__(self, n_outputs): # pass whatever arguments you need
        ...
        self.linear1 = nn.Linear(max_word_len + 2, linear_hidden_size)
        self.dropout = nn.Dropout(p=0.2)
        self.linear2 = nn.Linear(linear_hidden_size, n_outputs)
    ...
    def forward(self, chars, mask): # the main method that runs this module
        ...
        char_embeddings = self.embedding(chars)
        x = char_embeddings * mask.unsqueeze(-1)
        x = x.mean(dim=-1)
        x = self.linear1(x)
        x = F.relu(x)
        x = self.dropout(x)
...
class ImprovedNetwork(nn.Module):
    def __init__(self, n_outputs): # pass whatever arguments you need
        ...
        self.embedding = nn.Embedding(n_embeddings, embedding_size)
        self.linear1 = nn.Linear(2 * embedding_size, linear_hidden_size)
        self.dropout = nn.Dropout(p=0.2)
        self.linear2 = nn.Linear(linear_hidden_size, n_outputs)
    ...
    def forward(self, chars, mask):
        ...
        char_embeddings = self.embedding(chars)
        first_char_cutoff_indices = [0] + list(range(2, max_word_len + 2))
        embeddings_first_char_cutoff = char_embeddings[:, first_char_cutoff_indices, :]

        last_char_indices = mask.sum(1) - 2
        last_char_cutoff_indices = []
        for i in last_char_indices:
          all_indices = [i for i in range(max_word_len + 2)]
          all_indices.pop(int(i))
          last_char_cutoff_indices.append(all_indices)

        last_char_cutoff_indices = torch.tensor(last_char_cutoff_indices).cuda()
        last_char_cutoff_indices = last_char_cutoff_indices.unsqueeze(-1).expand(-1, -1, char_embeddings.size(-1))
        embeddings_last_char_cutoff = char_embeddings.gather(1, last_char_cutoff_indices)

        mask = mask[:, first_char_cutoff_indices]
        mask = mask.unsqueeze(-1)

        x1 = embeddings_first_char_cutoff * mask
        x2 = embeddings_last_char_cutoff * mask
